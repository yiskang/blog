...
- Deployment happens two ways, both landing on `gh-pages`: (1) manual, via `hexo-deployer-git` per the `deploy:` block in `_config.yml` (`npx hexo deploy`, requires local SSH access); (2) automatic, via `.github/workflows/deploy.yml`, which runs `hexo generate` and pushes `public/` to `gh-pages` using `peaceiris/actions-gh-pages` with the repo's built-in `GITHUB_TOKEN` on every push to `master`. The CI workflow needs the repo's Settings → Actions → General → Workflow permissions set to "Read and write permissions" or the push will fail with 403.
